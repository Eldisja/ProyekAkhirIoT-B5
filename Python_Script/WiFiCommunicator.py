    ...
    def __listener_thread(self):
        while not self._rip:
            ...
            if decoded_msg is not None:
                # Tambahkan langkah-langkah komparasi plat nomor di sini
                plat_nomor_terdeteksi = decoded_msg.data
                plat_nomor_diinginkan = "ABC123"  # Hardcode nilai plat nomor yang diinginkan
                ...
                if is_plat_nomor_sesuai:
                    # Kirim sinyal untuk palang bedas menyala
                    sinyal = OutMessage(data="Palang Bedas Menyala")
                    self._outgoing_messages_queue.put(sinyal)
                else:
                    # Kirim sinyal untuk palang bedas mati
                    sinyal = OutMessage(data="Palang Bedas Mati")
                    self._outgoing_messages_queue.put(sinyal)

                # Tambahkan pesan yang telah didekode ke dalam antrian pesan masuk
